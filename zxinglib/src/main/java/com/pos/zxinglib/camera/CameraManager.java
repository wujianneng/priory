package com.pos.zxinglib.camera;
...
public final class CameraManager {
	...
	public void openOrCloseFlashLight(boolean canLight,Context context) {
		boolean hasCamera = DeviceUtil.checkCameraHardware(context);
		boolean hasFlashLight = DeviceUtil.checkCameraHardwareHasFlashLight(context);
	  if(hasCamera){
		if (canLight) {
			if(hasFlashLight){
			  if (camera == null){
			  ...
			  }else if(camera.getParameters().getFlashMode().equals(Camera.Parameters.FLASH_MODE_OFF)) {
				Camera.Parameters parameter = camera.getParameters();
				parameter.setFlashMode(Camera.Parameters.FLASH_MODE_TORCH);
				camera.setParameters(parameter);
			  } else {
				...
				Toast.makeText(context,"该设备没摄像头", Toast.LENGTH_LONG).show();
			}
		}
	  }
	}
	...
	public Rect getFramingRect(Context context) {
		...
		if (framingRect == null) {
			...
			double w = DeviceUtil.getScreenWidth(context);
			double h = DeviceUtil.getScreenHeight(context);
			if(DeviceUtil.isScreenOriatationPortrait(context)) {
				Log.e("test","Portrait");
				framingRect = new Rect((int) (w * 0.15), (int) (h * 0.2), (int) (w * 0.85)
						, (int) (h * 0.58));
			}else {
				Log.e("test","noPortrait");
				framingRect = new Rect((int) (w * 0.2), (int) (h * 0.1), (int) (w * 0.8)
						, (int) (h * 0.7));
			}
//			framingRect = new Rect(leftOffset, topOffset - 200, leftOffset
//			+ width, topOffset + height - 200);
	 ...
	 * frame, not UI / screen.
	 */
	public Rect getFramingRectInPreview(Context context) {
