...
public class DinghuoAdapter extends BaseQuickAdapter<ProductSelectResultBean, BaseViewHolder> {

    public DinghuoAdapter(@LayoutRes int layoutResId, @Nullable List<ProductSelectResultBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, ProductSelectResultBean item) {
        helper.setText(R.id.name_tv, item.getName());
        helper.setText(R.id.weight_tv, item.getWeight() + "g");
        helper.setText(R.id.code_tv, item.getProductcode() + "");
        helper.setText(R.id.price_tv, item.getUnitprice() + "元");
